...
// ```c
// // demo.c
// // vc.c expects render() to be defined and also supplies it's own entry point
...
// $ clang -fno-builtin --target=wasm32 --no-standard-libraries -Wl,--no-entry -Wl,--export=render
// -Wl,--allow-undefined -o demo.wasm -DVC_PLATFORM=VC_WASM_PLATFORM demo.c
// ```
...
static void vc_term_resize_char_canvas(size_t new_width, size_t new_height) {
    ...
    // Or we can resize the frame on the fly similarly to how we resize sprites in olivec_sprite_*()
    // functions.
    ...
    vc_term_char_canvas = malloc(sizeof(*vc_term_char_canvas) * vc_term_scaled_down_width *
...
static void vc_term_compress_pixels(Olivec_Canvas oc) {
    ...
    for (size_t y = 0; y < vc_term_scaled_down_height; ++y) {
        for (size_t x = 0; x < vc_term_scaled_down_width; ++x) {
            Olivec_Canvas soc =
                olivec_subcanvas(oc, x * VC_TERM_SCALE_DOWN_FACTOR, y * VC_TERM_SCALE_DOWN_FACTOR,
...
int main(void) {
    for (;;) {
        ...
        for (size_t y = 0; y < vc_term_scaled_down_height; ++y) {
            for (size_t x = 0; x < vc_term_scaled_down_width; ++x) {
                // TODO: explore the idea of figuring out aspect ratio of the character using escape
                // ANSI codes of the terminal and rendering the image accordingly
